...
| Джойстик | Проводной орган управления, подключённый к Модулю (интерфейс I2C) |
| Мёртвая рука | Кнопка на джойстике, разрешающая движение только при удержании |
...
| Ключ | Трёхпозиционный ключевой переключатель Модуля: OFF / LOCAL / AUTO |
...
| I2C | Джойстик | SDA GPIO21, SCL GPIO22 |
| SPI | Дисплей ST7789 | SCK GPIO18, MOSI GPIO23, RES GPIO16, DC GPIO17, CS GPIO5 |
| GPIO вход | Энкодер: вращение | CLK GPIO32, DT GPIO33 |
| GPIO вход | Энкодер: нажатие (кнопка «OK» / «Загрузить») | SW GPIO27 |
...
Ключ трёхпозиционный последовательный: переходы возможны только между соседними положениями (OFF↔LOCAL↔AUTO). Прямой переход OFF↔AUTO физически невозможен.
...
- **FR-40.4.** Кнопка «Пуск» — орган моментного действия; удержание не требуется и не имеет отдельного смысла.
...
- **FR-37.1.** При потере связи с джойстиком по I2C (заданное число неудачных транзакций подряд) прошивка должна немедленно перевести выход управления в нейтраль и отобразить ошибку.
- **FR-37.2.** При зависании шины I2C прошивка должна выполнять процедуру восстановления шины (bus recovery).
...
stateDiagram-v2
    ...
    LOCAL --> OFF: ключ → OFF
    LOCAL --> AUTO: ключ → AUTO
    AUTO --> LOCAL: ключ → LOCAL
...
stateDiagram-v2
    ...
    Drive --> JoyFault: потеря I2C
    Ready --> JoyFault: потеря I2C
...
6. **Читаемость дисплея** в условиях эксплуатации (открытая площадка, прямой солнечный свет). При недостаточной читаемости — пересмотр состава индикации.
