 ...
 *
 * После пересборки в новом корпусе (2026-09-хх) стенд показал, что
 * физически GPIO39 отвечает за влево/вправо, а GPIO36 — за
 * вперёд/назад, то есть наоборот тому, что подразумевали имена X/Y.
 * Поменяно местами здесь программно (без перепайки): X = влево-вправо
 * (GPIO39), Y = вперёд-назад (GPIO36). */
#define MODULE_JOYSTICK_X_GPIO      39
#define MODULE_JOYSTICK_Y_GPIO      36
 ...
 * монтажа.
 *
 * Поднято до штатных 40 МГц (2026-09-хх, пульт пересобран в новом
 * корпусе, макетных проводов-джамперов больше нет) — на стенде на
 * макетке 5 МГц занижало SPI настолько, что полная перерисовка кадра
 * блокировала ui_task на ~180 мс за раз (320x170x16bpp / 5 МГц), и
 * опрос кнопки энкодера проваливался в эту паузу — часть коротких
 * тапов физически не долетала до debounce. Если на экране появится
 * "снег"/помехи — вернуть ближе к прежнему значению (например,
 * 20 МГц) и переоценить монтаж. */
#define MODULE_DISPLAY_SPI_HZ       (40 * 1000 * 1000)
...
/* Центр каждой оси в сырых отсчётах ADC (12 бит, 0..4095). На стенде
 * 2026-09-18 (Приложение Б, п. 5) совпадал с серединой шкалы (2048).
 * После пересборки пульта в новом корпусе перемерено заново
 * (2026-09-хх, 31 замер в покое, ДО перестановки X/Y выше): каналу
 * GPIO39 (теперь X) соответствовал центр ≈1815, каналу GPIO36
 * (теперь Y) — ≈1849. TODO(stand): перепроверить, если корпус ещё раз
 * разберут — крепление джойстика явно не жёстко фиксирует ноль. */
#define MODULE_JOYSTICK_ADC_CENTER_X       1815
#define MODULE_JOYSTICK_ADC_CENTER_Y       1849

/* Направление осей: 1 — инвертировать знак после центрирования.
 * Стенд (2026-09-хх): вправо уже даёт положительное значение без
 * инверсии; вперёд без инверсии давало отрицательное — включена
 * инверсия по Y, чтобы «от себя» было положительным. */
#define MODULE_JOYSTICK_X_INVERT           0
#define MODULE_JOYSTICK_Y_INVERT           1
